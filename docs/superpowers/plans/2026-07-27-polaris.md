...
Cada página de subgênero termina com o bloco **"Como usar esta página"**, que hoje existe no Notion e é conteúdo próprio de cada seção. Ele mora no corpo Markdown do arquivo de subgênero correspondente à seção, sob um cabeçalho de nível 2, e é renderizado no rodapé.
...
Run: `npx vitest run && npm run build && cd scripts && python -m pytest migracao/tests/ -v`
Expected: tudo passa
